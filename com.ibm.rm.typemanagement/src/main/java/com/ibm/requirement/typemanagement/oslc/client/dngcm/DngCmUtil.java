...
public class DngCmUtil {
	 ...
	 * 			Do not use.
	 * 
	 *             This is an internal API that is not officially supported. Use of
	 *             this API is unsupported and the API can change any time without
	 *             notice.
	 * 
	 *             Use
	 *             {@code com.ibm.requirement.typemanagement.oslc.client.resources.Changeset.Changeset(JazzFormAuthClient, Configuration) }
	 * 
	 *             Get the URL to create change sets
	 * 
	 * @param client
	 * @return
	 */
	public static String getChangeSetCreationFactory(JazzFormAuthClient client) {
		return client.getUrl() + "/localVersioning/configurations/changesets";
	}
	 ...
	 *             {@code com.ibm.requirement.typemanagement.oslc.client.resources.Changeset.Changeset(JazzFormAuthClient, Configuration) }
	 * 
	 *             Create a change set for a configuration
	 * 
	 * @param client
	 * @param target
	 * @return
	 ...
	 * @throws IOException
	 */
	public static Configuration createChangeSetConfiguration(final JazzFormAuthClient client, final String target)
			throws IOException, OAuthException, URISyntaxException {
		if (target == null) {
			logger.info("Target must not be null");
			return null;
		}
		String changeSetURI = createChangeSet(client, target);
		if (null == changeSetURI) {
			return null;
		}
		return DngCmUtil.getConfiguration(client, changeSetURI);
	}
	 ...
	 * 
	 * 
	 *             Create a change set for a target configuration URI (e.g. a
	 *             Stream)
	 * 
	 * @param client
	 * @param target
	 * @return
	 */
	public static String createChangeSet(final JazzFormAuthClient client, final String target) {
		...
		final String changesetFactotryURL = getChangeSetCreationFactory(client);

		// Create JSON object for creation factory
		JsonObject changeSet = Json.createObjectBuilder()
				.add("name", "Import-Type-System-CS-" + TimeStampUtil.getTimestamp()).add("configurationId", target)
				.build();
		...
		addHeader.put(DngTypeSystemManagementConstants.DNG_CM_CONFIGURATION_CONTEXT_HEADER, target);

		logger.debug("CreateChangeSet");
		logger.trace("CreateChangeSet using factory '{}'", changesetFactotryURL);
		logger.trace("Json '{}'", changeSet.toString());

		DngHeaderRequestInterceptor.installRequestInterceptor(client, addHeader);
		ClientResponse response = null;
		try {
			response = client.createResource(changesetFactotryURL, changeSet.toString(),
			 ...
			 * 
			 * {"changeSetUri":"https://clm.example.com:9443/rm/cm/changeset/_y0hwwO2hEeiuaLG8tKK07w"}
			 *
			 * 
			 */
			switch (response.getStatusCode()) {
			case 200:
				...
				JsonObject createdChangeSet = jsonReader.readObject();
				if (!createdChangeSet.containsKey("changeSetUri")) {
					return null;
				}
				changeSetUri = createdChangeSet.getString("changeSetUri");
				logger.debug("The change set URI is '{}'.", changeSetUri);
				break;
			default:
